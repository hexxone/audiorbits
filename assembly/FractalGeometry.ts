...
let scaleY: f32;
// let tmp: f32;
let s: i32; // subset counter
let x: f32; // p-coord
let y: f32; // p-coord
let newX: f32;
let newY: f32;
let i: i32; // point counter
let z: f32; // alg-param
let x1: f32; // alg-param
let bid: i32; // overall level index
let dist: f32;
let scaling: f32;
let outer: f32;
let ldnum: f32; // subset depth
let lData: Float32Array = new Float32Array(0);
let tmpSet: f32;
let levelSpiral: f32;
let levelCount: i32;
    ...
    for (s = 0; s < numSubsets; s++) {
        ...
        for (i = 0; i < numPoints; i++) {
            if (i < hlfPoints || choice1 > 0.3) {
                // first half of points: Iteration formula (generalization of Barry Martin's one)

                if (choice1 < 0.5) {
                    z = dl + isqrt(iabs(bl * x - cl));
                } else if (choice1 < 0.75) {
                    z = dl + isqrt(isqrt(iabs(bl * x - cl)));
                } else {
                    z = dl + ilog(2 + isqrt(iabs(bl * x - cl)));
                }
                ...
                if (levelSpiral > 0.5) {
                    rotatePoint(
                        x,
                        y,
                        -(levelId as f32) * deg45rad
                            + ((s as f32) * deg45rad) / (levelCount as f32)
                    );
                } else {
