...
import { CComponent,
    ...
    WascInterface,
    LoadHelper,
    sharedWorker } from 'we_utils/src';
...
export class GeometryHolder extends CComponent {
    ...
    private async getGeoBuilder(): Promise<WascInterface> {
        return new Promise((res, rej) => {
            ...
            if (cachedBuilders[builderString]) {
                // use from cache?
                res(cachedBuilders[builderString]);
            } else {
                ...
                sharedWorker(builderString)
                    .then((myModule) => {
                        ...
                        if (myModule.shared) {
                            Smallog.debug(
                                'Got shared memory access to WebAssembly builder!'
                            );
                        }
        ...
        for (let l = 0; l < sett.num_levels; l++) {
            ...
            for (let s = 0; s < sett.num_subsets_per_level; s++) {
                ...
                if (sett.level_shifting) {
                    ...
                    if (l % 2 !== 0) {
                        object.position.z -= subsetDist;
                    }
                } else {
                    object.position.z = lDist - s * subsetDist;
                }

                // centered around camera?
                if (sett.xr_mode) {
                    object.position.z += hlfDepth;
                }
     ...
     * @returns {Object3D} result subset
     */
    ...
    private getSeed(): number {
        ...
        return Math.abs(this.settings.random_seed) % 233280;
    }
    ...
    private updateWithAudio(ellapsed, deltaTime) {
        ...
        // calc audio boost
        const { lastAudio } = this.weas;
        const strengthMult = (20 + sett.audio_increase) / 110;
        const boost
            = lastAudio.intensity * strengthMult * (sett.equalize ? 4 : 1);
        ...
        const hues = this.colorHelpr.hueValues;
        const { color_mode } = this.colorHelpr.settings;
        const camZ = this.camera.position.z;
        ...
        for (lv = 0; lv < this.levels.length; lv++) {
            ...
            for (ss = 0; ss < level.sets.length; ss++) {
                ...
                freqIdx = Math.min(
                    lastAudio.data.length,
                    Math.max(0, stepDist - 2)
                );
                ...
                if (color_mode === 4) {
                    targetHue
                        += ((colObject.hueB - targetHue) * freqVal)
                        / lastAudio.max;
